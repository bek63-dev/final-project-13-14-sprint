// Package domain реализует бизнес-логику планировщика: вычисление даты
// следующего выполнения задачи по правилу повторения (ежедневно,
// еженедельно, ежемесячно, ежегодно). Используется пакетом api при
// валидации и обработке задач.
package domain
...
func NextDate(now time.Time, startDate string, repeat string) (string, error) {
	...
	// Диспетчеризация по типу правила повторения
	switch ruleType {
	case RuleTypeDays:
